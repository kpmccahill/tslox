...
function defineAst(outputDir: string, baseName: string, types: Array<string>): void {
    ...
    source += "\n"
    source += `export interface ${baseName} {\n`
    source += "    accept<T>(visitor: Visitor<T>): T;\n"
    source += "}\n\n"
...
function defineVisitor(baseName: string, types: string[]): string {
    ...
    for (var type in types) {
        ...
        source += ""
        source += `    visit${typeName}${baseName}(${baseName.toLowerCase()}: ${typeName}): T\n`
    }
    source += "}\n"
...
function defineType(baseName: string, className: string, fieldList: string): string {
    let source: string = `export class ${className} implements ${baseName} {\n`
    source += buildTypeFields(fieldList) + "\n"
    source += buildTypeConstructor(fieldList) + "\n"
    source += buildTypeAccept(baseName, className)
    source += "}\n\n"
...
function buildTypeAccept(baseName: string, className: string): string {
    let source = ""
    source += `    accept<T>(visitor: Visitor<T>): T {\n`
    source += `        return visitor.visit${className}${baseName}(this)\n`
    source += `    }\n`
    return source
}
